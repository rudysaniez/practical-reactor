...
class c4_LifecycleHooks extends LifecycleHooksBase {
    ...
    void terminator() {
        ...
        Flux<Integer> temperatureFlux = room_temperature_service()
                //todo: change this line only
                .log()
                .doOnCancel(() -> System.out.println(" > Hey there is a cancel signal !"))
                .doOnTerminate(hooksTriggeredCounter::incrementAndGet)
                ;

        // Finish by cancel, the take operator is used
        StepVerifier.create(temperatureFlux.take(2))
                    .expectNextCount(2)
                    .verifyComplete();

        StepVerifier.create(temperatureFlux.skip(15))
                    .expectNextCount(5)
                    .verifyComplete();
