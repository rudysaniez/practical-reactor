...
class c5_CreatingSequence {
    ...
    void value_I_already_have_mono() {
        ...
        Mono<String> valueIAlreadyHaveMono = Mono.fromCallable(() -> valueIAlreadyHave); //todo: change this line only
     ...
     *
     * Answer:
     * - What is difference between `generate` and `create`? : `generate` operator is a synchronous sink, and the `create` operator is asynchronous sink
     * - What is difference between `create` and `push`? : `create` operator is asynchronous sink (multiple thread)
     * and the `push` operator can emit multiple elements in a same thread.
     */
    @Test
    void generate_programmatically() {
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            ...
            for(int i=0 ; i <= 5; i++) {
                var iLocal = i;
                Thread.startVirtualThread(() -> sink.next(iLocal));
            }
        });
        ...
        StepVerifier.create(createFlux.take(6).sort())
                    .expectNext(0, 1, 2, 3, 4, 5)
                    .verifyComplete();
    ...
    void multi_threaded_producer() {
        ...
        Flux<Integer> producer = Flux.create(sink -> {
            for (int i = 0; i < 100; i++) {
                int finalI = i;
                Thread.startVirtualThread(() -> sink.next(finalI));
