...
class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    void i_am_rubber_you_are_glue() {
        ...
        var data = Flux.concat(numberService1(), numberService2());
    ...
    @Test
    void instant_search_other() {

        Flux<String> suggestions = userOtherSearchInput()
                .switchMap(this::autoComplete);

        StepVerifier.create(suggestions)
                .expectNext("reactor project", "rx-programming project")
                .verifyComplete();
    }
    ...
    void acid_durability() {
        ...
        Flux<String> committedTasksIds = tasksToExecute()
                .concatMap(task -> task.flatMap(s -> commitTask(s)
                                                .thenReturn(s))
                );
        ...
        /*
            With flatMapSequential
         */
        committedTasksCounter.set(0);

        committedTasksIds = tasksToExecute()
                .flatMapSequential(task -> task.flatMap(s -> commitTask(s)
                        .thenReturn(s))
                );
    ...
    void car_factory() {
        ...
        Another example
         */
        producedCars = carChassisProducer()
        ...
        Another example
         */
        producedCars = Flux.zip(carChassisProducer(), carEngineProducer(), Car::new);
